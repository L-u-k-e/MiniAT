...
variable_line ::= VAR_DIRECTIVE IDENTIFIER(id). {
	if(pass == 1) {
		...
		if(!symbol_table_search(constants, id->token_str)) {
			addr = (inline_variables ? &variable_address.u : &address.u);
...
inv_code_line(val) ::= INV(inst) REGISTER(reg). {
	set_instruction(&val, 0, 0, 0, M_EXOR, reg->data.u, reg->data.u, 0, ~0, inst->line_num);
}
...
flush_code_line(val) ::= FLUSH(inst). {
	set_instruction(&val, 0, 0, 0, M_BRAE, 0, 0, M_REGISTER_PC, 0, inst->line_num);
}
