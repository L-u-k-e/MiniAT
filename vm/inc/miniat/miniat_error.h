 ...
 * subsystem.
 *
 * Note: If you add a new error, you should update it in the error map in error.c
 */
enum m_error_num {
	m_error_num_none = 0,             //!< No error
	m_error_num_illegal_argument,     //!< Illegal argument
	m_error_num_double_init,          //!< Double initialization occurred
	m_error_num_out_of_range,         //!< Out of range
	m_error_num_invalid_error_num,    //!< Invalid error number
	m_error_num_invalid_binary,       //!< Invalid binary file
	m_error_num_pc_queue_full,        //!< An add to to PC was attempted with a full PC queue
	m_error_num_reg_queue_full,       //!< An add to the register hazard queue was attempted but the queue was full
	m_error_num_mem_queue_full,       //!< An add to the memory hazard queue was attempted but the queue was full
	m_error_num_pc_queue_empty,       //!< A remove to the pc queue was attempted but the queue was empty
	m_error_num_reg_queue_empty,      //!< A remove to the register hazard queue was attempted but the queue was empty
	m_error_num_mem_queue_empty,      //!< A remove to the memory hazard queue was attempted but the queue was empty
	m_error_num_null_pointer,         //!< Null pointer was passed as an argument
	m_error_num_barn_fire,            //!< A crazy error has occurred and we don't know how to handle it
	m_error_num_out_of_mem,           //!< No memory left
	m_error_num_error_already_handled,//!< Trying to handle an error that was already handled
	/*
	 * This one must be last to accurately count how many errnums there are.
	 */
	m_error_num_cnt                   //!< Number of errors
};

typedef enum m_error_num m_error_num;
 ...
 */
#define TRY \
	{ int _e_; m_error_stack __env = { 0, 0 }; __error_push(&__env); switch( _e_ = setjmp(__env.jbuf) ){ case 0: while(1){
 ...
 * @warn
 *  This version drops TLS as its not supported
 *  by all platforms. This differs us from upstream
 *  cexception, but who really cares :-P
 */
extern m_error __m_err__;
 ...
 *  The error code
 */
extern void __set_active_error(int line, const char *file, const char *func, m_error_num e);
