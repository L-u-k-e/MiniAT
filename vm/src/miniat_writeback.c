...
void m_writeback_stage(miniat *m) {
	...
	case M_STORE:
		...
		if(m->pipeline.writeback.state == m_pipeline_stage_state_no_hazard){
			...
			if(valid.type != m_memory_type_peripheral) {
				...
				stall_duration = valid.duration;

				if(stall_duration != M_NO_STALL_DURATION) {

					/* Count this stage as an attempt to make forward progress*/
					m_pipeline_stalls_set(m, m_pipeline_stage_writeback, m_pipeline_stage_state_structural_memory_hazard, --stall_duration);
				}
				else {
					 ...
					 */
					if(valid.type == m_memory_type_csr && ((((D == M_CSR_INT_ENABLE_LOW) ||
							(D == M_CSR_INT_ENABLE_HIGH)) && m->csr->fi) ||
					((D == M_CSR_SYSTEM_REGISTER) && m->csr->fs))) {
						m_pipeline_flush(m);
				...
				m_pipeline_stalls_set(m, m_pipeline_stage_writeback, m_pipeline_stage_state_structural_peripheral_hazard, 0);
		...
		else if(m->pipeline.writeback.state == m_pipeline_stage_state_structural_memory_hazard && --m->pipeline.writeback.duration <= 0) {
			/*
		...
		else if(m->pipeline.writeback.state == m_pipeline_stage_state_structural_peripheral_hazard) {
			/*
	...
	case M_IRET:
		...
		*(m->interrupts.ifr_h) &= ~(m->interrupts.irr_h);
		...
		memcpy(&(m->pipeline.register_hazards), &(m->interrupts.ivbt[M_IVBT_PC].register_hazards), sizeof(m->pipeline.register_hazards));
		memcpy(&(m->pipeline.memory_hazards), &(m->interrupts.ivbt[M_IVBT_PC].memory_hazards), sizeof(m->pipeline.memory_hazards));
		memcpy(&(m->pipeline.decode.in), &(m->interrupts.ivbt[M_IVBT_PC].decode), sizeof(m->pipeline.decode.in));
		memcpy(&(m->pipeline.execute.in), &(m->interrupts.ivbt[M_IVBT_PC].execute), sizeof(m->pipeline.execute.in));
		memcpy(&(m->pipeline.writeback.in), &(m->interrupts.ivbt[M_IVBT_PC].writeback), sizeof(m->pipeline.writeback.in));
