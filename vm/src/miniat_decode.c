...
void m_decode_stage(miniat *m) {
	...
	case M_MOD:
		...
		/*Else there is a hazard and you must set the hazard and push NOPs*/
			...
			m_pipeline_stalls_set(m, m_pipeline_stage_decode, m_pipeline_stage_state_data_hazard, 0);
		}
		break;
	case M_LOAD:
		...
		if(!(m_hazards_check(m, rC, true) || m_hazards_check(m, D, false))) {
			...
			if((valid.permissions == m_memory_permissions_read_access || valid.permissions == m_memory_permissions_full_access) && ((valid.type == m_memory_type_ram) || (valid.type == m_memory_type_flash)
					|| (valid.type == m_memory_type_id) || (valid.type == m_memory_type_csr) || (valid.type == m_memory_type_gpio) || (valid.type == m_memory_type_ivt)
	...
	case M_STORE:
		if(!(m_hazards_check(m, rC, true) || m_hazards_check(m, rA, true))) {
			 ...
			 */
			valid = m_memory_address_lookup(D);
			if(((valid.permissions == m_memory_permissions_write_access) || (valid.permissions == m_memory_permissions_full_access))
					&& ((valid.type == m_memory_type_ram) || (valid.type == m_memory_type_flash) || (valid.type == m_memory_type_csr)
					|| (valid.type == m_memory_type_gpio) || (valid.type == m_memory_type_ivt) || (valid.type == m_memory_type_peripheral))) {
				m_pipeline_stalls_clear(m, m_pipeline_stage_decode);
	...
	case M_BRAGE:
		...
		if(!(m_hazards_check(m, rA, true) || m_hazards_check(m, rB, true) || m_hazards_check(m, rC, true))) {
			m_pipeline_stalls_clear(m, m_pipeline_stage_decode);

			valid = m_memory_address_lookup(D);
			if((valid.type == m_memory_type_flash || valid.type == m_memory_type_ram)
					&& (valid.permissions == m_memory_permissions_read_access || valid.permissions == m_memory_permissions_full_access)) {
				if(hint) {
	...
	if(!m_hazards_check(m, predicate_register, true)) {
		...
		/* If their are no interrupts, the instruction is not predicated and the pipeline is not in a hazard (ie all is well)
		   then go ahead and send everything to the output latches. */
		if(!int_tripped && !predicate_value && m->pipeline.decode.state == m_pipeline_stage_state_no_hazard) {
		...
		else if(!int_tripped && (predicate_value || m->pipeline.decode.state != m_pipeline_stage_state_no_hazard)) {
