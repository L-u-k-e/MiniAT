...
void m_execute_stage(miniat *m) {
	...
	switch (opcode) {
		...
		case M_LOAD:
				...
				if(valid.type != m_memory_type_peripheral) {
					...
					if(stall_duration != M_NO_STALL_DURATION) {
						m_pipeline_stalls_set(m, m_pipeline_stage_execute,
								m_pipeline_stage_state_structural_memory_hazard, --stall_duration);
					}
					else {
			 ...
			 */
			else if(m->pipeline.execute.state == m_pipeline_stage_state_structural_memory_hazard &&
					--m->pipeline.execute.duration <= 0) {
			 ...
			 */
			else if(m->pipeline.execute.state == m_pipeline_stage_state_structural_peripheral_hazard) {
				/*
			...
			break;
		case M_STORE: result.u = A.u;
			break;
		case M_RLOAD: m_registers_read(m, D.u, &result.u);
			break;
		case M_RSTORE: result.u = A.u; rX = D.u;
			break;
		case M_AND: result.s = B.s & D.s;
			break;
		case M_OR: result.u = B.u | D.u;
			break;
		case M_EXOR: result.u = B.u ^ D.u;
			break;
		case M_SHL: result.u = B.u << D.u;
			break;
		case M_SHR: result.u = B.u >> D.u;
			break;
		case M_BRAE: result.u = A.u == B.u;
			break;
		case M_BRANE: result.u = A.u != B.u;
			break;
		case M_BRAL:
			if(m->csr->u) {
				result.u = A.u < B.u;
	...
	if(m->pipeline.execute.state == m_pipeline_stage_state_no_hazard && !int_tripped) {
		out->D = D.u;
		out->hint = in->hint;
		out->opcode = opcode;
		out->rX = rX;
