...
void m_timers_new(miniat *m) {
	...
	if(m) {
		...
		timers[m_timer_id_0].compare = (m_timer_compare *)&(m->mem[M_CSR_TIMER0_COMPARE]);
		timers[m_timer_id_1].compare = (m_timer_compare *)&(m->mem[M_CSR_TIMER1_COMPARE]);
		timers[m_timer_id_2].compare = (m_timer_compare *)&(m->mem[M_CSR_TIMER2_COMPARE]);
		timers[m_timer_id_3].compare = (m_timer_compare *)&(m->mem[M_CSR_TIMER3_COMPARE]);

		/*initialize General Purpose Controls*/
		control = (m_timer_control *)&(m->mem[M_CSR_TIMER_CONTROL]);
		...
		/*int the IO*/
		timer_io = (m_timers_io *)&(m->mem[M_CSR_TIMER_IO]);
		...
		timers[m_timer_id_0].count = &(m->mem[M_CSR_TIMER0_COUNT]);
		timers[m_timer_id_1].count = &(m->mem[M_CSR_TIMER1_COUNT]);
		timers[m_timer_id_2].count = &(m->mem[M_CSR_TIMER2_COUNT]);
		timers[m_timer_id_3].count = &(m->mem[M_CSR_TIMER3_COUNT]);

		/*Initialize Watchdog Compare*/
		watchdog->compare = (m_watchdog_compare *)&(m->mem[M_CSR_WATCHDOG_COMPARE]);
		watchdog->control = (m_watchdog_control *)&(m->mem[M_CSR_WATCHDOG_CONTROL]);
	...
	for(i = 0; i < M_NUM_OF_TIMERS; i++) {
		...
		if(timer->control->reset) {
			...
			case m_timer_pwm:
				...
				/*
				 * For example a frequency of 10 with a 3 cycle duty would produce the following waveform...
				 *
				 *            level ---_______
				 * pre-scaled cycle 0123456789
				 */
				if(*(timer->count) >= timer->compare->frequency) {
	...
	if(m) {
		 ...
		 */
			...
			if(wd->control->count >= wd->compare->value) {
				...
				m_memory_read(m, M_IVT_WATCHDOG, &tmp);

				if(tmp) {
					interrupt = M_INT_WATCHDOG;
				}
				else {
					interrupt = M_INT_RESET;
				}
				m_memory_read(m, M_CSR_INT_FLAG_LOW, &tmp);
				tmp |= 1 << interrupt;
				m_memory_write(m, M_CSR_INT_FLAG_LOW, tmp);
