...
static void debugger_interrupts_changed(debugger *d, miniat *m);
static void debugger_timers_changed(debugger *d, miniat *m);
static void debugger_pipeline_changed(debugger *d, miniat *m);
static void debugger_bus_changed(debugger *d, miniat *m);
static void debugger_csr_changed(debugger *d, miniat *m);
static void debugger_ticks_changed(debugger *d, miniat *m);
...
void debugger_mem_changed(debugger *d, miniat *m) {
	...
	for(; d->mem_i < M_MEMORY_SIZE; d->mem_i++) {
		if(d->m.mem[d->mem_i] != m->mem[d->mem_i]) {
			return;
		}
	}
	return;
...
int debugger_timer_state(debugger *d, miniat *m) {
	...
	for(i=0; i < m_timer_id_cnt ; i++) {
                t = &(m->timers.general_purpose_timer[i]);
                        printf("    %08x", t->compare->frequency);
        }
        printf("\n");
